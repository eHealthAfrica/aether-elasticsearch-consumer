...
def test__find_child_attr(SimpleSchema):
    matches = [i for i in SimpleSchema.find_children({'has_attr': ['default']})]
    assert(len(matches) == 2)


@pytest.mark.unit
def test__find_child_attr__failure(SimpleSchema):
    matches = [i for i in SimpleSchema.find_children({'has_attr': ['non-existant']})]
    assert(len(matches) == 0)


@pytest.mark.unit
def test__find_child_match_attr(SimpleSchema):
    matches = [i for i in SimpleSchema.find_children({'match_attr': [{'name': '_id'}]})]
    assert(len(matches) == 1)


@pytest.mark.unit
def test__find_child_match_attr__failure(SimpleSchema):
    matches = [i for i in SimpleSchema.find_children({'match_attr': [{'name': 'not-available'}]})]
    assert(len(matches) == 0)
